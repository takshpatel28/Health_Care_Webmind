...
import { FaImage, FaPaperPlane, FaTimes, FaRobot, FaUser, FaPlus, FaStethoscope, FaHistory, FaFilePdf } from 'react-icons/fa';
...
export default function DoctorChat() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            formData.append('message', message || (fileType === 'pdf' ? 'Please analyze this PDF' : 'Please analyze this image'));
            if (file) {
            ...
            const userMessage = {
                role: 'user',
                content: message || (fileType === 'pdf' ? 'PDF uploaded' : 'Image uploaded'),
                fileInfo: file ? {
                    name: file.name,
                    type: fileType,
                    size: file.size,
                ...
                if (session.id === currentSessionId) {
                    return {
                        ...
                        title: session.history.length === 0 
                            ? (message || (fileType === 'pdf' ? 'PDF query' : 'Image query')).substring(0, 20) + (message?.length > 20 ? '...' : '') 
                            : session.title
                    };
                }
                return session;
            ...
            <main className="flex-1 overflow-y-auto p-4 pb-24">
                <div className="max-w-3xl mx-auto space-y-4">
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-center py-12 animate-fade-in">
                            <div className="bg-white p-5 rounded-2xl shadow-sm mb-6 transform transition-all hover:scale-105">
                                ...
                                <p className="text-gray-600 max-w-md">
                                    Ask me anything about health, symptoms, or upload an image/PDF for analysis.
                                </p>
                            </div>
                    ...
                    {getCurrentChat()?.history.map((msg, index, messages) => {
                        const isUserFileMessage = msg.role === 'user' && 
                                                msg.fileInfo && 
                                    ...
                                    <div className={`px-4 py-3 rounded-xl ${msg.role === 'user' 
                                        ...
                                        {isUserFileMessage && (
                                            <div className="mt-2">
                                                ...
                                                {msg.content !== 'PDF uploaded' && msg.content !== 'Image uploaded' && (
                                                    <p className="whitespace-pre-wrap text-sm mt-2">{msg.content}</p>
                                                )}
                                            </div>
                                        )}
                                        {msg.isFileResponse && (
                                            <div className={`p-2 rounded-lg border ${msg.fileType === 'pdf' 
                                                ? 'bg-red-50 border-red-100' 
                                                : 'bg-blue-50 border-blue-100'}`}>
                                                <div className="text-xs mb-1">
                                                    {msg.fileType === 'pdf' ? 'PDF Analysis:' : 'Image Analysis:'}
                                                </div>
                                                <p className="whitespace-pre-wrap text-sm">{msg.content}</p>
                                            </div>
                                        )}
                                        {!isUserFileMessage && !msg.isFileResponse && (
                                            <p className="whitespace-pre-wrap text-sm">{msg.content}</p>
                                        )}
                                    </div>
                                </div>
            ...
            <footer className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 py-4 px-6 shadow-lg backdrop-blur-sm bg-opacity-90">
                <div className="max-w-3xl mx-auto">
                        ...
                        <div className="relative mb-3 bg-gray-50 rounded-xl p-3 flex items-center border border-gray-200 shadow-inner">
                            {fileType === 'image' ? (
                                ...
                                <div className="flex items-center p-2 bg-red-50 rounded-lg border border-red-200">
                                    <FaFilePdf className="text-red-600 mr-2" size={24} />
                                    <div>
                    ...
                    <div className="text-center mt-3 text-xs text-gray-500">
                        ...
                        <br />
                        Supported file types: JPEG, PNG, GIF, PDF (max 5MB)
                    </div>
                </div>
            </footer>
        </div>
