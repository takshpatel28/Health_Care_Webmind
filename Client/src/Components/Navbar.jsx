      ...
      <div className='container mx-auto px-4'>
        <div className='flex justify-between items-center'>
          ...
          <div className='flex items-center space-x-4'>
            {!user ? (
              <>
                <Link to="/login" className='hidden md:block px-4 py-2 text-sm font-medium text-white hover:text-blue-300 transition-colors duration-300'>
                  Login
                </Link>
                <Link to="/signup" className='hidden md:block px-6 py-2 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full text-sm font-medium text-white shadow-lg hover:shadow-xl transition-all duration-300 hover:from-blue-600 hover:to-purple-700'>
                  Sign Up
                </Link>
              </>
            ) : (
              <div className='relative'>
                <button onClick={() => setIsProfileOpen(!isProfileOpen)} className='flex items-center space-x-2 focus:outline-none group'>
                  <div className='relative'>
                    <img
          ...
          <div className='flex flex-col space-y-3 px-2'>
            ...
            {!user ? (
              <div className='flex flex-col space-y-3 pt-4'>
                <Link to="/login" className='w-full px-4 py-3 text-center bg-gray-700 rounded-lg text-white font-medium hover:bg-gray-600 transition-colors duration-300'>
                  Login
                </Link>
                <Link to="/signup" className='w-full px-4 py-3 text-center bg-gradient-to-r from-blue-500 to-purple-600 rounded-lg text-white font-medium hover:from-blue-600 hover:to-purple-700 transition-all duration-300'>
                  Sign Up
                </Link>
              </div>
            ) : (
...
const NavLink = ({ to, icon, text }) => (
  <Link to={to} className="flex items-center px-4 py-2 text-sm font-medium text-gray-300 hover:text-white rounded-lg group transition-colors duration-300">
    <span className="mr-2 opacity-70 group-hover:opacity-100 transition-opacity duration-300">{icon}</span>
    {text}
